...
BEGIN
    ...
    _phone := COALESCE(data->>'phone', NULL);
    _email := COALESCE(data->>'email', NULL);
    _dob := COALESCE(NULLIF(data->>'dob', '')::DATE, NULL);
    _currentAddress := COALESCE(data->>'currentAddress', NULL);
    _permanentAddress := COALESCE(data->>'permanentAddress', NULL);
    _fatherName := COALESCE(data->>'fatherName', NULL);
    ...
    _admissionDt := COALESCE(NULLIF(data->>'admissionDate', '')::DATE, NULL);
    _roll := COALESCE((data->>'roll')::INTEGER, NULL);

    RAISE NOTICE 'Parsed input: userId=%, name=%, email=%, dob=%, admissionDt=%, systemAccess=%',
      _userId, _name, _email, _dob, _admissionDt, _systemAccess;
    ...
    RAISE NOTICE 'Operation: %', _operationType;
    ...
    RAISE NOTICE 'Assigned reporterId: %', _reporterId;

    IF NOT EXISTS(SELECT 1 FROM users WHERE id = _userId) THEN
        RAISE NOTICE 'User does not exist, creating new one';

        IF EXISTS(SELECT 1 FROM users WHERE email = _email) THEN
            RAISE NOTICE 'Email % already exists', _email;
        ...
        RAISE NOTICE 'Student added: %', _userId;
    ...
    RAISE NOTICE 'Updating user: %', _userId;
    ...
    RAISE NOTICE 'Student updated: %', _userId;
